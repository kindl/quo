...
importStatement = liftA2 Import
    ...
    (optional (curlies (sepByTrailing identifier (token ","))))
...
forPart =
    liftA3 (,,) identifier optionalType (token "in" *> expr)
...
definition =
    liftA3 Definition (token "let" *> identifier) optionalType (token "=" *> expr)
...
parameter = liftA2 (,) identifier optionalType
